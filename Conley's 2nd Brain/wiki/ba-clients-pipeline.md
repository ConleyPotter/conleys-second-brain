---
type: operations
domain: ba
tags: [ba, day-job, clients, pipeline, hubspot]
created: 2026-04-16
updated: 2026-04-17
sources: [conley-weekly-summary-apr-6-12-2026.md, Daily-Recap_2026-04-15.md, Daily-Recap_2026-04-16.md]
---

# BA Clients & Pipeline

**Related:** [[ba-overview]], [[ba-products]], [[ba-partners]]

**This is a rolling page.** It documents key client relationships and pipeline accounts that appear repeatedly in Conley's recaps. HubSpot is the system of record for deal stages and exact amounts — this page captures the strategic and relational context that HubSpot deal cards don't hold.

Update this page in place as accounts evolve. Do not create a new page per client — log entries in [[log]] are for that.

---

## How to Read This Page

**Relationship types:**
- **Active client** — currently receiving BA services, paying monthly
- **New business** — in proposal or negotiation stage
- **AutoVitals co-managed** — BA + AutoVitals joint account; coordinated via the AV partner team
- **Prospect** — in pipeline but no proposal yet
- **Onboarding** — signed, not yet live

Accounts are organized by how recently they appeared in Conley's recaps and their strategic weight.

---

## High-Priority Accounts (Active or Near-Close)

### Heights Expert Automotive
**Contact:** Heather  
**Status:** Active client — Google Ads + LSA. Website deal in pipeline.  
**Recent activity:** Post-meeting recap sent Apr 15. Performance strong across Google Ads and LSA. Website decision is the main open item — BA to follow up in 1–2 weeks. BA owes: pricing deck, video production pricing, LSA targeting screenshots. Client owes: forward LSA emails, Apple Maps setup, AutoBoost testimonial.  
**Deal context:** Heather reconnected via email Apr 8 after the deal moved Closed Won; performance review rebooked Apr 13.

---

### Bernhard Auto Works
**Contact:** Dave Bernhard (owner); Andrea (partner/co-decision-maker)  
**Status:** New business — proposal sent, under internal review  
**Recent activity:** Monday Apr 14 new business meeting (Conley + Jon @ BA). Follow-up email sent Apr 15. Scope discussed: Google Ads, programmatic advertising, website, review management. Meeting heavily focused on attribution tracking, ROI measurement, and new vs. returning customer revenue — a recurring theme in new-business discussions. BA owes: programmatic case studies, ad placement list. Dave and Andrea reviewing proposal internally.  
**Context:** Shop owner facing staffing shortages — framed as a growth challenge, not just a marketing one.

---

### CarLink360
**Contact:** Chad  
**Status:** Decision Maker Bought-In  
**Deal value:** $19,884  
**Products:** Standard Website + Review Platform + Google Ads Jumpstart+ + Standard LSA + Social Content Curation  
**Recent activity:** Deal created Apr 8 after Chad's call led directly to a quote. Quote out; awaiting signature.

---

### Metric Motors
**Contact:** Gina  
**Status:** Prospect — active sales process; proposal in progress  
**Recent activity:** Multi-day scheduling thread; Conley escalated to Adam. Adam-led call on Apr 15 (49 min, Gina + Adam). Apr 16 — Adam posted three service tier options in #internal-sales, tagging Jon, Conley, and Ethan to build a sales deck: (1) Standard Website / Google Ads JumpStart+ / Bing; (2) Advanced Website / Google Ads Standard / Bing; (3) Advanced Website / Google Ads Advanced / Bing / Programmatic 2 Medium. Conley assigned programmatic ad placement examples to Ethan, Abby, and Courtney with links to the Google Slides deck and HubSpot template. Sales deck in progress.

---

### Turnkey multi-shop lead (unnamed)
**Contact:** Via Jared Baker @ Turnkey Auto Marketing  
**Status:** Inbound referral — Advanced LSA inquiry, not yet in HubSpot  
**Deal potential:** Multi-shop, Advanced LSA + likely Google Ads + website — flagged to Jon as "a huge stinking deal"  
**Recent activity:** Jared Baker emailed Apr 15 asking if BA does Advanced LSA. Conley sent Standard vs. Advanced breakdown. Jared confirmed he'd share with the prospect.

---

## Active Clients (Ongoing Relationships)

### Legacy Auto Care
**Contact:** Brian Peterson  
**Status:** Closed Won → New client (separate location is a new brand)  
**Deal value:** $5,985 (Standard Website, closed Apr 10)  
**Recent activity:** Same-day prospect call Apr 15 (31 min, flagged by Courtney Nguyen). New brand next door to existing BA client location. No prior information at time of call.

---

### Scandinavian Imports
**Contact:** Bogdan  
**Status:** Active client — LSA + landing page discussion in progress  
**Recent activity:** Post-meeting recap email sent Apr 10 (March performance + LSA/landing page info). From weekly recap: Conley handled a multi-day rescheduling thread with Bogdan around a late call Monday + OOO sick Tuesday.

---

### Salta
**Status:** Active client — performance issues  
**Recent activity:** Apr 15 — Conley flagged poor performance to Jon. Strategy discussion TBD. No resolution yet.

---

## AutoVitals Co-Managed Accounts

These clients are managed jointly with AutoVitals. See [[ba-partners]] for the AutoVitals relationship structure.

### STAR Automotive
**Status:** Active AutoVitals partner account  
**Recent activity (Apr 6–12):** Google Ads performance review (Apr 7). Conley asked Adam Hicks (BA's Paid Media Specialist) about Meta brand-awareness ads for technician hiring at STAR (Apr 10).

### Kidders Repair
**Contact:** Keith  
**Status:** Prospect — warm inbound from AutoVitals; introductory call held  
**Recent activity:** Apr 16 — "Connects with AV/AutoBoost" call attended (~9 min, Conley + Jon + Keith + Stephen Baker/AV). New AV client converting to add BA Google Ads services. Follow-up needed: confirm next steps and service tier with Keith.  
**Context:** Conley identified this lead on Apr 15 and asked Jon to schedule it for Thursday.

### Los Gatos Auto
**Contact:** David  
**Status:** Active AutoVitals partner account  
**Recent activity:** AutoVitals joint meeting Apr 8 (BA/AV + Los Gatos). Conley sent "Quick Meeting Follow-Up" covering March car count / new customers (Apr 10).

### Dixon Automotive
**Contact:** Paul  
**AV contact:** Carly Lama  
**Status:** Active AutoVitals partner account  
**Recent activity:** Monthly KPI review call Apr 9 (AutoVitals partner).

### East Coast Automotive
**Contact:** Larry  
**Status:** Active AutoVitals partner account — Google Ads  
**Recent activity:** Google Ads review call Apr 9. Conley sent "Jumpstart+ and Budget Estimates" follow-up based on 39 calls at $12.21/call analysis.

### Lael's International Auto Service
**Status:** Active AutoVitals partner account — budget reallocation just implemented  
**AV contact:** Shelby Maggard  
**Recent activity:** Conley requested 2-week delay on Apr 29 meeting (budget change just went live); Shelby said keep the meeting. Apr 29 confirmed.

### Exclusively Imports
**Contact:** Kevin  
**Status:** Active AutoVitals partner account  
**Recent activity:** 33-minute monthly review call Apr 15.

### Tilson's Auto Repair
**Contact:** Ben and Joe  
**Status:** New Deal: Closed — Prospect Dropped  
Still a current shared AutoVitals Advanced Google Ads client. May still add LSA.
**Recent activity:** Apr 16 — Conley and Jon mutually decided to move on. Conley: "he probably caught bad vibes from me. I thought that whole process was uncool of them in general." Jon agreed and reported the same vibe. Removed from active pipeline.  
**Context:** Was in "Decision Maker Bought-In" with LSA deal expected to close on AutoVitals paper. Steve @ AV had requested the call recording (Apr 10). Both parties chose not to re-engage with the broader deal involving a website and programmatic ads.

---

## Recently Closed / Onboarding

### Best One Fleet Service of Gas City
**Status:** Closed Won  
**Deal value:** $19,987 (Conley listed as owner; Jon as collaborator — commission split)  
**Closed:** Apr 6

### Rodgers Family Auto Care
**Status:** Onboarding — Jumpstart+ upgrade signed and paid  
**Recent activity:** Confirmed Jumpstart+ upgrade; signed and paid within the week of Apr 6–7.

### Harford County Transmissions & Repair: Chris Knopp & Paul Marsh 
**Status:** Signed, payment pending; onboarding not yet started  
**Recent activity (Apr 15):** Onboarding email sent. Directed to book kick-off call. Payment still outstanding from earlier in the month.

---

### A&R Complete Auto
**Contact:** Angie (sole contact as of Apr 16); Taina has departed  
**Status:** Active client — contact change; meeting rescheduling in progress  
**Recent activity:** Apr 16 — Angie emailed to cancel the 2 PM Zoom, noting Taina is no longer with A&R. Conley forwarded to Jon and took ownership of rescheduling. Angie available Fri 4/17 or Tue 4/22.

---

### Sav-Mor Automotive (Brenner Newman)
**Contact:** Brenner Newman  
**Status:** Active client — April promotional campaign in motion  
**Recent activity:** Apr 16 — Brenner called in slow; wants promos on site and Google Ads. Conley + Courtney coordinated campaign: $47 synthetic oil change, free A/C diagnostic + 10% off, April-only specials. Deliverables: website, Google Ads, social, blog (National Car Care Month angle). Jon adjusting Google Ads toward general repair spend.  
**Signal:** Conley's reaction ("even he's feeling it now") suggests broader seasonal slowdown reaching typically stable clients.

---

### Kennedy's
**Status:** Active client — call tracking overlay declined  
**Recent activity:** Apr 16 — Kennedy's declined BA's call tracking overlay; going back to OktoRocket for CRM and related services. Not offboarding from BA services otherwise. Ethan clarified the situation to Conley.

---

## Pipeline Watch List

Accounts in the pipeline with notable size or momentum, tracked here for context:

| Account                         | Stage                    | Amount  | Note                                                          |
| ------------------------------- | ------------------------ | ------- | ------------------------------------------------------------- |
| Turbo Tim's Anything Automotive | Decision Maker Bought-In | $31,348 | Largest active pipeline deal                                  |
| ValleyPro AutoHouse             | Decision Maker Bought-In | $7,587  |                                                               |
| EuroPro Autohaus                | Decision Maker Bought-In | $7,587  |                                                               |
| Mechanics of West Allis         | Booked                   | —       | Appointment ~2 weeks from Apr 16. Schedule noted as "slammed" |

---

## Accounts Needing Attention

| Account                             | Issue                                                | Status                                              |
| ----------------------------------- | ---------------------------------------------------- | --------------------------------------------------- |
| Menard Automotive                   | Webmaster unreachable; tracking installation blocked | Ethan tried calling; Conley committed to continuing |
| Salta                               | Poor ad performance; strategy TBD                    | Flagged to Jon Apr 15                               |

---

> **TODO:** This page was built from two sources (weekly recap Apr 6–12 and daily recap Apr 15). A significant portion of BA's full client base is not yet represented here. To fill gaps:

> **AI PROMPT (for HubSpot access):**
> *"Export a list of all active client companies in HubSpot with their deal stage, assigned deal owner, total contract value, and most recent activity date. Filter for deals in stages: Active Client, Closed Won (within last 6 months), Contract Sent, Decision Maker Bought-In."*
