...
|---------|------|-------|-------|
| [[dailychew-overview\|DailyChew.AI]] | `ConleyPotter/daily-chew-ai` | Next.js, Inngest, LangGraph, Cloudflare R2, Resend, Drizzle/Postgres | Backend feature-complete for v0.1 MVP core loop |
| [[the-grind-overview\|The Grind]] | `ConleyPotter/lgs-the-grind` | Vite, Phaser 3, TypeScript, Vitest | Bootstrapped; backlog seeded; no game code yet |
...
Dev-log recaps are written weekly by the Vault Keeper agent (Dev-Log Capture mode, Friday 5pm ET). Each recap synthesizes merged PRs, commits, releases, and CI runs from both product repos into a single archival `work-log` page. Rolling product overview pages are updated with each recap.
